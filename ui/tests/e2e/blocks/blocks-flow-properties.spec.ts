...
test.describe("Block editor — flow properties panel", () => {
    ...
    test("id and namespace are locked when editing an existing flow", async ({page}) => {
        // Both render as disabled inputs — no way to type into them
        ...
        expect(await idField.locator("input:not([disabled])").count()).toBe(0)
        expect(await nsField.locator("input:not([disabled])").count()).toBe(0)
    })
